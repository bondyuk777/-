...
(function() {
  ...
  function changeElements() {
    ...
    if (bottomTip && bottomTip.textContent !== "𝑹𝒆𝒎𝒂𝒌𝒆 𝒗𝟑") bottomTip.textContent = "𝑹𝒆𝒎𝒂𝒌𝒆 𝒗𝟑";
...
// ==UserScript==
// @name         Fullscreen Snowfall
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  Снег по всему экрану на всех сайтах
// @author       You
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function () {
    ...
    // ----- НАСТРОЙКИ -----
    const FLAKE_COUNT = 120;      // количество снежинок
    const FLAKE_SIZE_MIN = 2;     // минимальный размер
    const FLAKE_SIZE_MAX = 6;     // максимальный размер
    const FLAKE_SPEED_MIN = 0.5;  // минимальная скорость падения
    const FLAKE_SPEED_MAX = 2.0;  // максимальная скорость падения
    const WIND_STRENGTH = 0.5;    // сила "ветра" по X

    // Проверка, чтобы не создать второй раз
    if (window.__tmSnowRunning) return;
    window.__tmSnowRunning = true;
    ...
    canvas.style.zIndex = '999999';      // поверх всего
    ...
    function createFlake() {
        return {
            x: Math.random() * width,
            y: Math.random() * height - height, // чтобы появлялись сверху
            radius: random(FLAKE_SIZE_MIN, FLAKE_SIZE_MAX),
            speed: random(FLAKE_SPEED_MIN, FLAKE_SPEED_MAX),
            wind: random(-WIND_STRENGTH, WIND_STRENGTH),
    ...
    function draw() {
        ...
        for (let i = 0; i < flakes.length; i++) {
            ...
            // если ушёл вниз – перекидываем наверх
            if (f.y > height + f.radius) {
                flakes[i] = createFlake();
                flakes[i].y = -flakes[i].radius;
            }

            // если ушёл вбок – возвращаем
    ...
    // ----- ГОРЯЧАЯ КЛАВИША: отключить/включить снег (Ctrl+Shift+S) -----
    let visible = true;
    document.addEventListener('keydown', (e) => {
